...
$(document).ready(function () {
    $(document).on("click", "button[name='edu']", function () {
        var result = $("select[name='eduSortation']").val();
        if (result == null) {
            alert('학력구분을 선택해주세요');
        } else {
            ...
            `;
                if (schoolName == "") {
                    ...
                    let snames = [];
                    let scname = $("input[name='schoolName']").val();
                    snames.push(schoolName);
                    console.log(snames);
                    $('.eduinput').show();
                ...
                let graduationStatus = $("select[name='graduationStatus']").val();
                let eduContent = `
                <div id="edu_info">
                <div id="edu_school">${schoolName}</div>
